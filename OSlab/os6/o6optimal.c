...
// This Function helps in finding the frame that will not be used
// for the longest period of time in the future in ref_str[0 ... refStrLen - 1]
int predict(int ref_str[], int frame_items[], int refStrLen, int index, int frame_occupied) {
    ...
    for (int i = 0; i < frame_occupied; i++) {
        ...
        // return it immediately as it's the best
        if (j == refStrLen)
            return i;
    }
  
    // If none of the frame items appear in the reference string
...
void optimalPage(int ref_str[], int refStrLen, int frame_items[], int max_frames) {
    ...
    // Here we traverse through the reference string and check for misses and hits
    int hits = 0;
    for (int i = 0; i < refStrLen; i++) {
        ...
        // frame index where we need to do replacement for this incoming reference string item
        else {
    ...
    float hit_ratio = (float)hits / refStrLen;
    float miss_ratio = 1.0 - hit_ratio;
